    ...
    public class ReferralController : BaseController
    {
        ...
        public async Task<IActionResult> RegisterWithReferral(RegisterViewModel model, string returnUrl = null)
        {
            try
            {
                if (_configuration["IPByPassLogin"].ToString() == "False")
                {
                    ...
                    {
                        return Ok(new BizResponseClass { ReturnCode = enResponseCode.Fail, ReturnMsg = EnResponseMessage.IpAddressInvalid, ErrorCode = enErrorCode.Status4188RequestedRemoteIPNotFound });
                    }
                }
        ...
        public async Task<IActionResult> GetReferralURL()
        {
            try
            {
                ...
                //var shareURl = _ReferralChannel.SendReferralEmail();
                var shareURl = new ReferralChannelShareURLViewModel
                {
                    EmailURL = user.Email,
                    FacebookURL = user.Email,
                    GoogleURL = user.Email,
                    InstaURL = user.Email,
                    LinkedInURL = user.Email
                ,
                    MessengerURL = user.Email,
                    PintrestURL = user.Email,
                    QRCodeURL = user.Email,
                    SMSURL = user.Email,
                    TelegramURL = user.Email,
                    TwitterURL = user.Email,
                    WhatsAppURL = user.Email
                }; //share url in all Channel Address Add by jagdish
        ...
        public IActionResult AddReferralUserClick(AddReferralClickRequest Request)
        {
            try
            {
                if (string.IsNullOrEmpty(Request.PassURL))
                {

                    return Ok(new ReferralUserClickResponse
                    {
                        ReturnCode = enResponseCode.Fail,
                        ErrorCode = enErrorCode.Status14199PassDataNotFound,
                        ReturnMsg = EnResponseMessage.PassDataNotFound
                    });
                }
                //Set IpAddress Null Condition by Rushali(25-12-2019)
                if (HttpContext.Connection.RemoteIpAddress.ToString() != null)
                {
                ...
                {
                    return Ok(new ReferralUserClickResponse { ReturnCode = enResponseCode.Fail, ReturnMsg = EnResponseMessage.IpAddressInvalid, ErrorCode = enErrorCode.Status4188RequestedRemoteIPNotFound });
                }
                //Comment IPByPass Condition BY Rushali (25-12-2019)
                //if (_configuration["IPByPassLogin"].ToString() == "False")
                //{
                //    if (HttpContext.Connection.RemoteIpAddress.ToString() != null)
                //    {
