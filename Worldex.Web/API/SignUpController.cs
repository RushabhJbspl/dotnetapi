    ...
    public class SignUpController : BaseController
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            try
            {
                ...
                ///////////////// Check bizUser  table in MobileNumber  Exist or not
                ...
                var currentUser = new ApplicationUser   ///  Create the Bizuser table 
                {
                    ...
                    ReferralCode = "", // changes by jagdish parmar : 04-09-2019
                    Thememode = model.Thememode //Rita 29-6-19 given by devang bhai , come in registration
                };
                ...
                if (resultdata.Succeeded)
                {
                    ...
                    if (!string.IsNullOrEmpty(model.ReferralCode))
                    {
                        ...
                        ////////get Referral code as a Email id add by jagdish parmar : 04-09-2019
                        ////////Resion Refferal Code as Email id 
                 
                        var user = await _userManager.FindByEmailAsync(model.ReferralCode);

                        //check referalcode - mansi  04-10-2019 --commite bcz in worldex referralcode use as email so referral code validation not need
                        //int ReferUserExist = await _ReferralUser.GetReferCodeUser(model.ReferralCode);
                        //if (ReferUserExist == 0)
                        //{
                        //    return BadRequest(new SignUpWithMobileResponse { ReturnCode = enResponseCode.Fail, ReturnMsg = EnResponseMessage.Status4189InValidReferralCode, ErrorCode = enErrorCode.Status4189InValidReferralCode });
                        //}
                    ...
                    //Rushabh 09-01-2020 Changed From resultMobileUserName.Id To currentUser.Id As resultMobileUserName has always null value for new user.
                    OtpMasterViewModel OptData = await _otpMasterService.AddOtpForSignupuser(currentUser.Id, null, model.Mobile);

                    // khushali 15-11-2019 check phone number verified
                    if (_configuration["SignUPWithMobile"].ToString() == "true")
                    {
                        SendSMSRequest smsrequest = new SendSMSRequest();
                        smsrequest.MobileNo = Convert.ToInt64(model.Mobile);
                        smsrequest.Message = EnResponseMessage.SendMailBody + OptData.OTP;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(result?.Email))
                {
                    ...
                    if (result.EmailConfirmed == false)
                    {
                        ...
                        linkToken.Id = result.Id;
                        linkToken.Email = model.Email;
                        linkToken.Lastname = result.LastName;
                        linkToken.Mobile = result.Mobile;
                        linkToken.CurrentTime = DateTime.UtcNow;
                        linkToken.Expirytime = DateTime.UtcNow + TimeSpan.FromHours(2);
                        linkToken.CountryCode = result.CountryCode;
                        linkToken.Device = DeviceDetails != null ? DeviceDetails[0] : string.Empty;
                        linkToken.DeviceOS = DeviceDetails != null ? DeviceDetails[1] : string.Empty;
                        linkToken.DeviceID = DeviceDetails != null ? DeviceDetails[2] : string.Empty;
                        linkToken.IpAddress = model.IPAddress;
        ...
        public async Task<IActionResult> SignUpWithEmail(SignUpWithEmailViewModel model, string returnUrl = null)
        {
            try
            {
                if (_configuration["IPByPassLogin"].ToString() == "False")
                {
                    ...
                    {
                        return Ok(new SignUpWithEmailResponse { ReturnCode = enResponseCode.Fail, ReturnMsg = EnResponseMessage.IpAddressInvalid, ErrorCode = enErrorCode.Status4188RequestedRemoteIPNotFound });
                    }
                }
                var language = _LanguagePreferenceMaster.GetSingle(i => i.PreferedLanguage == model.PreferedLanguage);
                if (language == null)
            ...
            a:
                ...
                if (UserCreate.Succeeded)
                {
                    ...
                    if (OptData != null)
                    {
                        ...
                        var resultotp = await _otpMasterService.GetOtpData(Convert.ToInt32(OptData.UserId), Convert.ToInt32(enRegisterType.Email)); // khushali 25-07-2019 change enum for mobile idetify for mobile - remove static 1 and 2.
                        // khushali 30-01-2019 for Common Template Method call 
                        TemplateMasterData TemplateData = new TemplateMasterData();
                        CommunicationParamater communicationParamater = new CommunicationParamater();
                        SendEmailRequest request = new SendEmailRequest();
        ...
        public async Task<IActionResult> EmailOtpVerification(OTPWithEmailViewModel model)
        {
            try
            {
                ...
                if (resultUser?.Id > 0)
                {
                    ...
                    // Need to Change here. -Nishit Jani on A 2019-07-06 5:29 PM
                    var otp = await _otpMasterService.GetOtpData(Convert.ToInt32(resultUser.Id), Convert.ToInt32(enRegisterType.Email)); // khushali 25-07-2019 change enum for mobile idetify for mobile - remove static 1 and 2.
                    if (otp != null)
                    {
        ...
        public async Task<IActionResult> ReSendOtpWithEmail(SignUpWithEmailViewModel model)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(result?.Email))
                {
                    ...
                    // Need to Change here. -Nishit Jani on A 2019-07-06 5:29 PM
                    var otp = await _otpMasterService.GetOtpData(Convert.ToInt32(result.Id), Convert.ToInt32(enRegisterType.Email)); // khushali 25-07-2019 change enum for mobile idetify for mobile - remove static 1 and 2.
                    if (otp != null)
                    {
                        _otpMasterService.UpdateEmailAndMobileOTP(otp.Id);
                        var resultdata = await _otpMasterService.AddOtpForSignupuser(Convert.ToInt32(result.Id), result.Email);
        ...
        public async Task<IActionResult> MobileOtpVerification(OTPWithMobileViewModel model)
        {
            try
            {
                ...
                // Need to Change here. -Nishit Jani on A 2019-07-06 5:29 PM
                OtpMasterViewModel OtpData = await _otpMasterService.GetOtpData(resultUserName.Id, Convert.ToInt32(enRegisterType.Mobile)); // khushali 25-07-2019 change enum for mobile idetify for mobile - remove static 1 and 2.
                ...
                if (OtpData != null)
                {
                    if (OtpData?.ExpirTime >= DateTime.UtcNow)
                    {
                        ...
                        {
                            // khushali 15-11-2019 check phone number verified also max try attampt
                            if (resultUserName?.AccessFailedCount < Convert.ToInt16(_configuration["MaxFailedAttempts"]))
                            {
                                resultUserName.AccessFailedCount = resultUserName.AccessFailedCount + 1;
                                if (resultUserName.AccessFailedCount == Convert.ToInt16(_configuration["MaxFailedAttempts"]))
                                {
                                    resultUserName.LockoutEnd = DateTime.UtcNow.AddHours(Convert.ToInt64(_configuration["DefaultLockoutTimeSpan"]));
                                    resultUserName.AccessFailedCount = 0;
        ...
        public async Task<IActionResult> ReSendOtpWithMobile(SignUpWithMobileViewModel model)
        {
            try
            {
                ...
                // Need to Change here. -Nishit Jani on A 2019-07-06 5:29 PM
                var otp = await _otpMasterService.GetOtpData(Convert.ToInt32(resultUserName.Id), Convert.ToInt32(enRegisterType.Mobile)); // khushali 25-07-2019 change enum for mobile idetify for mobile - remove static 1 and 2.

                if (otp != null)
                {

                    if (!resultUserName.PhoneNumberConfirmed)
                    {
                        _otpMasterService.UpdateEmailAndMobileOTP(otp.Id);
