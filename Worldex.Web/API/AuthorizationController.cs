    ...
    public class AuthorizationController : ControllerBase
    {
        ...
        public async Task<IActionResult> Exchange(OpenIdConnectRequest request, string appkey)
        {
            ...
            a:
                ...
                //}
                var remoteip = HttpContext.Features.Get<IHttpConnectionFeature>()?.RemoteIpAddress?.ToString();
                var remoteip2 = HttpContext.Connection.RemoteIpAddress.ToString();
               
                Worldex.Core.Helpers.HelperForLog.WriteLogForActivity("~/connect/token", " User ID : " + user.Id.ToString(), " remoteip :" + remoteip + " remoteip2 :" + remoteip2);

                    //komal 19-06-2019 validate IP
                    if (_configuration["IPByPass"].ToString() == "true" && user.IsDeviceAuthEnable == 1)//Mansi 03-10-2019 add DeviceAuth bit for front user preference
                    {
