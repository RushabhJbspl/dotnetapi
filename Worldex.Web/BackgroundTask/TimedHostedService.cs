...
namespace Worldex.Web.BackgroundTask
{
    ...
    internal class TimedHostedReleaseAndStuckOrderService : IHostedService, IDisposable
    {
        ...
        private Timer _timerForArbitrageLP;
        private readonly IMediator _mediator;
        private readonly IConfiguration _configuration;

        public TimedHostedReleaseAndStuckOrderService(ILogger<TimedHostedReleaseAndStuckOrderService> logger, IMediator mediator, IConfiguration configuration)
        {
            _logger = logger;
            _mediator = mediator;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogInformation("Timed Background Service is starting.");
                int Second = Convert.ToInt32(_configuration["ReleaseArbitrageLPOrderTime"]);
                ...
                _timerForArbitrageLP = new Timer(ReleaseArbitrageLOrder, null, TimeSpan.Zero,
        ...
        private void ReleaseArbitrageLOrder(object state)
        {
            try
            {
                var StuckOrderObj = new ReleaseArbitrageLPOrder()
                {
                    DateTime = Helpers.UTC_To_IST().AddSeconds(10)
                };
        ...
        public Task StopAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                _timerForArbitrageLP?.Change(Timeout.Infinite, 0);
                return Task.CompletedTask;
            }
            catch (Exception ex)
            {
