...
namespace Worldex.Web.BackgroundTask
{
    internal class TimedHostedLPStatusCheck : IHostedService, IDisposable
    {
        ...
        public TimedHostedLPStatusCheck(ILogger<TimedHostedService> logger, IMediator mediator, IConfiguration configuration)
        {
            _logger = logger;
            _mediator = mediator;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            try
            {
                int Second = Convert.ToInt32(_configuration["LPStatusCheck"]);
