...
using System.Linq;
using Worldex.Infrastructure.LiquidityProvider.CEXIO;
...
{
    public class CancelOrderProcessV1 : ICancelOrderProcessV1
    {
        ...
        private readonly IKrakenLPService _krakenLPService; //Add new variable for Kraken Exchange by Pushpraj as on 03-07-2019
        ...
        private readonly IEXMOLPService _eXMOLPService; //Add new variable for Exmo Exchange by Pushpraj as on 17-07-2019



        //PoolOrder PoolOrderObj;//komal 03 May 2019, Cleanup
        string ControllerName = "CancelOrderProcessV1";
        decimal CancelOrderPrice = 0, CancelOrderQty = 0, TotalQty = 0;

        public CancelOrderProcessV1(EFCommonRepository<TradeTransactionQueue> TradeTransactionRepository,
            ICommonRepository<TradeCancelQueue> TradeCancelQueueRepository, ICancelOrderRepository ICancelOrderRepository, IUpbitService upbitService,
            ...
            IGetWebRequest IGetWebRequest, BinanceLPService BinanceLPService,
            BitrexLPService BitrexLPService, ICoinBaseService CoinBaseService, IPoloniexService PoloniexService,
            ITradeSatoshiLPService TradeSatoshiLPService, ICommonRepository<TransactionRequest> TransactionRequest,
            ITradeReconProcessV1 tradeReconProcessV1,
            IOKExLPService oKExLPService, IHuobiLPService huobiLPService, IMemoryCache cache, IKrakenLPService krakenLPService ,
            IBitfinexLPService bitfinexLPService,IYobitLPService yobitLPService,IEXMOLPService eXMOLPService,ICEXIOLPService cEXIOLPService)
        {
            ...
            _oKExLPService = oKExLPService; //Add new variable assignment for OKEx API by Pushpraj as on 1-06-2019
            _bitfinexLPService = bitfinexLPService;
            _cEXIOLPService = cEXIOLPService;
            _cache = cache;
            _krakenLPService = krakenLPService; //Add new variable assignment for Kraken Exchange by Pushpraj as on 03-07-2019
            _yobitLPService = yobitLPService; //Add new variable assignment for Yobit Exchagne by Pushpraj as on 16-07-2019
            _eXMOLPService = eXMOLPService;//Add new variable assignment for Exmo Exchagne by Pushpraj as on 17-07-2019
        }
        ...
        public async Task<BizResponse> ProcessCancelOrderAsyncV1(NewCancelOrderRequestCls Req)
        {
            ...
            //{
                _Resp = await MiddleWareForCancellationProcess(Req);
                Task.Run(() => HelperForLog.WriteLogIntoFile("ProcessCancelOrderAsyncV1 Code " + _Resp.ErrorCode, ControllerName, _Resp.ReturnMsg + " ##TrnNo:" + Req.TranNo));

                if (_Resp.ReturnCode == enResponseCodeService.Fail)//on validation fail send Notification
                {
                    try
                    {
                        if (Req.CancelOrderCount == 1)
                        {
                            ...
                            notification.Param1 = Req.TranNo.ToString(); //komal 31-01-2019  change Obj.TrnNo
        ...
        public async Task<BizResponse> MiddleWareForCancellationProcess(NewCancelOrderRequestCls Req)
        {
            ...
            try
            {
                ...
                {
                    _Resp = await CancellationProcessV1(_Resp, TransactionQueueObj, TradeTranQueueObj, Req.accessToken,Req.CancelOrderCount);
                }
                else//API trade
                {
                    _Resp = await CancellationProcessAPIV1(_Resp, TransactionQueueObj, TradeTranQueueObj, Req.accessToken, Req.CancelOrderCount);
                }
        ...
        public async Task<BizResponse> CancellationProcessV1(BizResponse _Resp, TransactionQueue TransactionQueueObj, TradeTransactionQueue TradeTransactionQueueObj, string accessToken, int OrderCount = 1)
        {
            ...
            try
            {
                ...
                try
                {
                    if (OrderCount == 1)
                    {
                        ...
                        await _ISignalRService.OnStatusCancel(TradeTransactionQueueObj.Status, TransactionQueueObj, TradeTransactionQueueObj, "", MakeTradeStopLossObj.ordertype, ISPartialSettled, OrderCount);
        ...
        public async Task<BizResponse> CancellationProcessAPIV1(BizResponse _Resp, TransactionQueue TransactionQueueObj, TradeTransactionQueue TradeTransactionQueueObj, string accessToken,int OrderCount = 1)
        {
            short ISPartialSettled = 0;
            CancelOrderPrice = 0; CancelOrderQty = 0;
            TradeBuyerListV1 BuyerListObj = new TradeBuyerListV1();
            ...
            try
            {
                ...
                {
                    _Resp.ErrorCode = enErrorCode.CancelOrder_APICallFail;
                    _Resp.ReturnCode = enResponseCodeService.Fail;
        ...
        public async Task<LPCancelOrdedrResponse> LiquidityTradeCancelProcess(long TrnNo, TradeTransactionQueue tradeTransactionObj)
        {
            ...
            try
            {
                ...
                TransactionRequest TransactionResponse = _TransactionRequest.FindBy(e => e.TrnNo == tradeTransactionObj.TrnNo).ToList().FirstOrDefault();
                ...
                TransactionRequest Request = _TranRequestRepository.FindBy(e => e.TrnNo == TrnNo).FirstOrDefault();
                if (Request == null)
                {
                ...
                var ServiceProConfiguration = _IGetWebRequest.GetServiceProviderConfiguration(LPProvidetail.Id);
                ...
                switch (LPProvidetail.AppTypeID)
                {
                    ...
                    //Add new case for Kraken Exchange by Pushpraj as on 03-07-2019
                    case (long)enAppType.Kraken:
                        var KrakenResult = await _krakenLPService.CancelOrderAsync(LPOredrID);
                        APIResponse = JsonConvert.SerializeObject(KrakenResult);
                        if (KrakenResult != null)
                        {
                            if (KrakenResult.pending == true)
                            {
                                _Res.TrnRefNo = KrakenResult.ToString();
                                _Res.ErrorCode = enErrorCode.Success;
                                _Res.TrnNo = TrnNo;
                                goto SuccessTrade;
                            }
                            ...
                            {
                                _Res.ReturnMsg = KrakenResult.pending.ToString();
                                _Res.ErrorCode = enErrorCode.API_Respose_Fail;
                                _Res.ReturnCode = enResponseCodeService.Fail;
                                _Res.TrnNo = TrnNo;
                                return _Res;
                    ...
                    case (long)enAppType.Bitfinex:
                        BitfinexGlobalSettings.API_Key = ServiceProConfiguration.APIKey;
                        BitfinexGlobalSettings.Secret = ServiceProConfiguration.SecretKey;
                        BitfinexGlobalSettings.PassPhrase = ServiceProConfiguration.Param1;                        
                        var BitfinexResult = await _bitfinexLPService.cancelOrderAsync("12345");
                        APIResponse = JsonConvert.SerializeObject(BitfinexResult);
                        if(BitfinexResult.is_cancelled == true)
                        {
                            _Res.TrnRefNo = BitfinexResult.id.ToString();
                            _Res.ErrorCode = enErrorCode.Success;
                            _Res.TrnNo = TrnNo;
                            goto SuccessTrade;
                        }
                        ...
                        //Add new case for Yobit Exchange by Pushpraj as on 16-07-2019
                    case (long)enAppType.Yobit:
                        ...
                        var YobitResult = await _yobitLPService.CancelOrder(LPOredrID);
                        APIResponse = JsonConvert.SerializeObject(YobitResult);
                        if (YobitResult.success == 1)
                        {
                            _Res.TrnRefNo = YobitResult.result.order_id.ToString();
                            _Res.ErrorCode = enErrorCode.Success;
                            _Res.TrnNo = TrnNo;
                            goto SuccessTrade;
                        }
                        ...
                        {
                            _Res.ReturnMsg = YobitResult.error.ToString();
                            _Res.ErrorCode = enErrorCode.API_Respose_Fail;
                            _Res.ReturnCode = enResponseCodeService.Fail;
                            _Res.TrnNo = TrnNo;
                            return _Res;
                        }
                    //Add new case for Exmo Exchange by Pushpraj as on 17-07-2019
                    case (long)enAppType.EXMO:
                        ...
                        var EXMOResult = await _eXMOLPService.CancelOrder(TrnNo.ToString());
                        APIResponse = JsonConvert.SerializeObject(EXMOResult);
                        if (EXMOResult.result == true)
                        {
                            _Res.TrnRefNo = LPOredrID;
                            _Res.ErrorCode = enErrorCode.Success;
                            _Res.TrnNo = TrnNo;
                        ...
                        {
                            _Res.ReturnMsg = EXMOResult.error.ToString();
                            _Res.ErrorCode = enErrorCode.API_Respose_Fail;
                            _Res.ReturnCode = enResponseCodeService.Fail;
                            _Res.TrnNo = TrnNo;
                            return _Res;
                    ...
                    case (long)enAppType.CEXIO:
                        CEXIOGlobalSetting.API_Key = ServiceProConfiguration.APIKey;
                        CEXIOGlobalSetting.Secret = ServiceProConfiguration.SecretKey;
                        CEXIOGlobalSetting.PassPhrase = ServiceProConfiguration.Param1;
                        var Cexioresult = await _cEXIOLPService.CancelOrder("89067468");
                        APIResponse = JsonConvert.SerializeObject(Cexioresult);
                        if (Cexioresult== true)
                        {
                          //  _Res.TrnRefNo = BitfinexResult.id.ToString();
                            _Res.ErrorCode = enErrorCode.Success;
