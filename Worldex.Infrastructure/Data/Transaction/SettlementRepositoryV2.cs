...
namespace Worldex.Infrastructure.Data.Transaction
{
    public class SettlementRepositoryV2: ISettlementRepositoryV1<BizResponse>
    {
        ...
        private readonly WorldexContext _dbContext1;
        ...
        public SettlementRepositoryV2(WorldexContext dbContext, //ICommonRepository<TradePoolQueue> TradePoolQueue,
        ...
        public async Task<BizResponse> PROCESSSETLLEMENTBuy(BizResponse _Resp, TradeTransactionQueue TradeTransactionQueueObj,TransactionQueue TransactionQueueObj, TradeStopLoss TradeStopLossObj, TradeBuyerListV1 CurrentTradeBuyerListObj,string accessToken = "",short IsCancel=0)
        {
            ...
            try
            {
                ...
                marketMakerUserRole = _IFrontTrnService.GetMarketMakerUserRole(); //komal 21-11-2019 get market maker user id
                ...
                foreach (TradeSellerListV1 SellerList in MatchSellerListBase)//MatchSellerList
                {
                    ...
                    //Rita 9-7-19 in continue overwrite occured so, remove entity modified
                    Qry = @"Update TradeSellerListV1 SET IsProcessing=1,UpdatedBy={1},CreatedBy={2},UpdatedDate=dbo.GetISTDate() WHERE TrnNo={0}";
                    var res = _dbContext.Database.ExecuteSqlCommand(Qry, SellerList.TrnNo, CurrentTradeBuyerListObj.TrnNo,800);
                    ...
                    if (SellerList.RemainQty < CurrentTradeBuyerListObj.RemainQty && SellerList.RemainQty != 0)//Do not Release Qty here //rita 8-2-18 release may occured so add <>0 Condition//Do not Release Qty here
                    {
                        ...
                        if (await WalletResult == false)
                        {
                            ...
                            //await WalletFail;//rita 17-10-19 remoce order cancellation
                        }
                        else
                        {
        ...
        public async Task<BizResponse> PROCESSSETLLEMENTSell(BizResponse _Resp, TradeTransactionQueue TradeTransactionQueueObj, TransactionQueue TransactionQueueObj, TradeStopLoss TradeStopLossObj, TradeSellerListV1 CurrentTradeSellerListObj, string accessToken = "", short IsCancel = 0)
        {
            ...
            List<ProccessSellWith> ProccessSellWithList = new List<ProccessSellWith>(); //Rita 09-7-19 not in used
            string Qry = "";
            try
            {
                ...
                MatchBuyerListBase = MatchBuyerListBaseAll.Where(item => (item.Price >= CurrentTradeSellerListObj.Price)//||item.OrderType==2 Rita 28-12-18 after updating price no need of this //&& item.IsProcessing == 0
                                                     ...
                                                     //&& (item.Status == Convert.ToInt16(enTransactionStatus.Initialize) || item.Status == Convert.ToInt16(enTransactionStatus.Hold))
                                                     && item.RemainQty > 0).OrderByDescending(x => x.Price).ThenBy(x => x.TrnNo); // Nishit Jani on A 2019-09-18 for logic modification test.
                                                                                                                                  //&& item.RemainQty > 0).OrderBy(x => x.TrnNo).OrderByDescending(x => x.Price);
                ...
                foreach (TradeBuyerListV1 BuyerList in MatchBuyerListBase)//MatchSellerList
                {
                    ...
                    //Rita 9-7-19 in continue overwrite occured so, remove entity modified
                    Qry = @"Update TradeBuyerListV1 SET IsProcessing=1,UpdatedBy={1},CreatedBy={2},UpdatedDate=dbo.GetISTDate() WHERE TrnNo={0}";
                    var res = _dbContext.Database.ExecuteSqlCommand(Qry, BuyerList.TrnNo, CurrentTradeSellerListObj.TrnNo, 800);
                    ...
                    if (BuyerList.RemainQty < CurrentTradeSellerListObj.RemainQty && BuyerList.RemainQty != 0)//Do not Release Qty here //rita 8-2-18 release may occured so add <>0 Condition
                    {
                        ...
                        GSettlementQty = SettlementQty;                        
                        SettlementPrice = BuyerList.Price; //komal set Buyer Price
                        BaseCurrQty = Helpers.DoRoundForTrading(SettlementQty * SettlementPrice,18);
                        GBaseCurrQty = BaseCurrQty;
                        //await SetIsProResult;
            ...
            try
            {
                ...
                if (CreditWalletResult.ReturnCode != enResponseCode.Success)
                {
                        ...
                        || CreditWalletResult.ErrorCode == enErrorCode.CrDrCredit_SettledBalMismatchDrWalletSecDr)
                    {
                        ...
                        _dbContext.Entry(TradeTQMakerObj).Reload();//rita 9-7-19 for latest value, with no overwrite
                        ...
                        _dbContext.Entry(SellerList).Reload();//rita 9-7-19 for latest value, with no overwrite
            ...
            try
            {
                ...
                if (CreditWalletResult.ReturnCode != enResponseCode.Success)
                {
                        ...
                        || CreditWalletResult.ErrorCode == enErrorCode.CrDrCredit_SettledBalMismatchDrWallet)
                    {
                        ...
                        _dbContext.Entry(BuyerList).Reload();//rita 9-7-19 for latest value, with no overwrite
        ...
        public async Task<bool> ReleaseWalletAmountSell(TradeSellerListV1 CurrentTradeSellerListObj, TransactionQueue TransactionQueueObj, TradeTransactionQueue TradeTransactionQueueObj,short IsPartial = 0)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        _ISignalRService.OnStatusCancel(TradeTransactionQueueObj.Status, TransactionQueueObj, TradeTransactionQueueObj, "", TradeTransactionQueueObj.ordertype, IsPartial);
                        //komal 8-10-2020 stop send email notification as client requirement through Ticket assign by Bhushanbhai 
        ...
        private async Task<bool> CheckIsProcessingBUY(long TrnNo,TradeSellerListV1 SellerList,decimal LTP)
        {
            try
            {
                ...
                if (SellerList.IsProcessing == 1 || SellerList.RemainQty <= 0 || (SellerList.Status != 4 && SellerList.Status != 0) || SellerList.IsAPITrade == 1)//rita 31-1-19 for cancellation , this order does not pick, Rita 14-10-19 add IsAPITrade condition for API+Local settlement issue
                {
                    Task.Run(() => HelperForLog.WriteLogIntoFileAsyncDtTm("CheckIsProcessingBUY Buy skip IsProcessing " + SellerList.IsProcessing + " Remain Qty " + SellerList.RemainQty + " Status:" + SellerList.Status + " With " + SellerList.TrnNo + " IsAPITrade:" + SellerList.IsAPITrade, ControllerName, "##TrnNo " + TrnNo, Helpers.UTC_To_IST()));
                    return false;
                }
        ...
        private async Task<bool> CheckIsProcessingSELL(long TrnNo, TradeBuyerListV1 BuyerList,decimal LTP)
        {
            try
            {
                ...
                if (BuyerList.IsProcessing == 1 || BuyerList.RemainQty <= 0 || (BuyerList.Status != 4 && BuyerList.Status != 0) || BuyerList.IsAPITrade == 1)//rita 31-1-19 for cancellation , this order does not pick, Rita 14-10-19 add IsAPITrade condition for API+Local settlement issue
                {
                    Task.Run(() => HelperForLog.WriteLogIntoFileAsyncDtTm("CheckIsProcessingSELL Sell skip IsProcessing " + BuyerList.IsProcessing + " Remain Qty " + BuyerList.RemainQty + " Status:" + BuyerList.Status + " With " + BuyerList.TrnNo + " IsAPITrade:" + BuyerList.IsAPITrade, ControllerName, "##TrnNo " + TrnNo, Helpers.UTC_To_IST()));
                    return false;
                }
        ...
        public async Task PartilaSettleBuySignalR(long TrnNo, TransactionQueue TransactionQueueObj,TradeTransactionQueue TradeTransactionQueueObj,TradeStopLoss TradeStopLossObj, TradeStopLoss MakeTradeStopLossObj, TransactionQueue TQMakerObj, TradeTransactionQueue TradeTQMakerObj,string accessToken,decimal SettlementPrice,decimal SettlementQty,short OrderType)
        {
            try
            {
                ...
                await _ISignalRService.OnLtpChange(SettlementPrice, TradeTQMakerObj.PairID, TradeTQMakerObj.PairName);//rita 11-1-19 for Stop and Limit add/remove in book
                ...
                //Rita 9-7-19 added only for one single record for one settlement instead of two settlement entry
                //_dbContext.Entry(TradeTQMakerObj).Reload();//for updated settlement date
                GetOrderHistoryInfo HistorySingleSettleRecord = new GetOrderHistoryInfo
                {
                    TrnNo = TradeTQMakerObj.TrnNo.ToString(),
                    DateTime = Convert.ToDateTime(TradeTQMakerObj.SettledDate),//partial settlement settlement date only in maker
                    Price = SettlementPrice,
                    SettlementPrice = SettlementPrice,
                    Amount = SettlementQty,
                    SettledQty = SettlementQty,
                    Type = "BUY",//only taker type
                    PairName = TradeTQMakerObj.PairName,
                    IsCancel = 0,
                    OrderType = TradeTransactionQueueObj.ordertype.ToString(),
                    SettledDate = Convert.ToDateTime(TradeTQMakerObj.SettledDate)//partial settlement settlement date only in maker
                };
                _ISignalRService.SendOrderHistory(HistorySingleSettleRecord, TradeTQMakerObj.PairName, TradeTQMakerObj.MemberID.ToString());
        ...
        public async Task FullSettleBuySignalR(long TrnNo, TransactionQueue TransactionQueueObj, TradeTransactionQueue TradeTransactionQueueObj,TradeStopLoss TradeStopLossObj, TradeStopLoss MakeTradeStopLossObj, TransactionQueue TQMakerObj, TradeTransactionQueue TradeTQMakerObj,string accessToken, decimal SettlementPrice, decimal SettlementQty,TradeSellerListV1 SellerList, short OrderType)
        {
            try
            {
                ...
                {
                    TrnNo = TradeTQMakerObj.TrnNo.ToString(),
                    DateTime = Convert.ToDateTime(TradeTransactionQueueObj.SettledDate),//full-settlement, settlementdate only in taker
                    Price = SettlementPrice,
                    SettlementPrice = SettlementPrice,
                    Amount = SettlementQty,
                    SettledQty = SettlementQty,
                    ...
                    IsCancel = 0,
                    OrderType = TradeTransactionQueueObj.ordertype.ToString(),
                    SettledDate = Convert.ToDateTime(TradeTransactionQueueObj.SettledDate),//full-settlement, settlementdate only in taker
                };
                _ISignalRService.SendOrderHistory(HistorySingleSettleRecord, TradeTQMakerObj.PairName, TradeTQMakerObj.MemberID.ToString());
            }
            catch (Exception ex)
        ...
        public async Task<bool> WalletProcessBuy(TradeTransactionQueue TradeTQueueCurrentObj, TradeTransactionQueue TradeTQueueMakerObj, decimal Price,decimal SettledQty,decimal BaseCurrQty,short CurrIsFullSettled,short MakerIsFullSettled, short TrnMode, WalletDrCrResponse CreditWalletResult1)
        {
        ...
        public async Task<bool> WalletProcessSell(TradeTransactionQueue TradeTQueueCurrentObj, TradeTransactionQueue TradeTQueueMakerObj, decimal Price, decimal SettledQty, decimal BaseCurrQty, short CurrIsFullSettled, short MakerIsFullSettled, short TrnMode, WalletDrCrResponse CreditWalletResult1)
        {
            CreditWalletResult = new WalletDrCrResponse();//here object pass not working in async method
            try
            {
